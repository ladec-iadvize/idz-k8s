...
- 2026-07-31 v3.4: exec-into-pod added (owner request — lifts the v3 out-of-scope note; 'shell' palette action, native SPDY+TTY, bash→sh fallback, first-ready-pod resolution for workloads/services) and CronJob manual trigger ('trigger' palette action, kubectl-style Job from template, confirmed). Node drain remains out of scope.
- 2026-07-31 v3.5: hierarchical drill-down (owner request — k9s-like). Enter walks one level DOWN the ownership/routing chain (workloads/Service → Pods → Containers, CronJob → Jobs → Pods, Ingress → Services, Node → Pods, Namespace → Pods) with a drill stack so Esc pops exactly one level; new containers view (state/ready/restarts/image, Enter = that container's logs, 'a' = shell into it). Extends FR-026 (navigate object relationships) downward — it previously covered ownership UP ('o') and a single workload→pods drill.

...
| RESTARTS column | Says **why** it restarted when the cluster knows (`4 (OOMKilled (exit 137))`) — an OOM kill is otherwise invisible on a pod that is running again. The containers view (Enter on a pod) has a full **LAST TERMINATION** column, and the pods list offers one in the `C` chooser |

...
| `> events` | Events **timeline**: a time axis per object, warnings highlighted (`w` = warnings only, `k` = kind filter, **`t` = time scale**: 5m/15m/1h/6h/24h/all — zoom in to separate a burst, out to see the whole retention); the header shows the **event and object counts** for the current scale; lanes take at most a third of the height so the detail list stays usable; **an OOMKilled pod is flagged on its event row** and the selected event names why that pod last stopped; the selected event's message shows in full below the list, `Enter` opens the referenced object |
...
| `> helm releases` | Helm releases: sortable/filterable list; `Enter` opens the release — **history with the chart/app version of each revision**, the deployed resources with their **live state**, the chart metadata (description, subcharts, deprecation), its **hooks with their last run**, **NOTES.txt** and the user values. In the release detail: `↑`/`↓` select a resource, `Enter` shows **its chart-rendered definition**, `y` its **live object**, `v` toggles values-only |
| Why a pod stopped (OOM…) | In the **events timeline**: an OOMKilled pod is flagged on its event row and the selected event names the reason. Also in the **containers view** (Enter on a pod) as a `LAST TERMINATION` column, and as an opt-in column of the pods list (`C` chooser). RESTARTS stays a plain kubectl-style count |

...
| `> events` | Events **timeline**: a time axis per object, warnings highlighted (`w` = warnings only, `k` = kind filter, **`t` = time scale**: 5m/15m/1h/6h/24h/all — zoom in to separate a burst, out to see the whole retention); the lane count and the detail list grow with the terminal height; the selected event's message shows in full below the list, `Enter` opens the referenced object |
